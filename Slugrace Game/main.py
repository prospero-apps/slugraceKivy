...
class Game(ScreenManager):
    ...
    time_set = StringProperty('') # string representation of the duration

    time_elapsed_delta = timedelta()   # duration
    time_elapsed = StringProperty('')  # string representation of the duration
 
    time_remaining_delta = timedelta()   # duration
    time_remaining = StringProperty('')  # string representation of the duration
    ...
    def __init__(self, **kwargs):
        ...
        self.background_music = SoundLoader.load('assets/Sounds/Game/Background Music.mp3')  
        self.slugs_running_sound = SoundLoader.load('assets/Sounds/Game/Slugs Running.mp3')
        self.gameover_sound = SoundLoader.load('assets/Sounds/Game/Game Over.mp3')
        self.go_sound = SoundLoader.load('assets/Sounds/Game/Go.mp3')

        # sounds assigned to particular slugs
        self.speedster.win_sound = SoundLoader.load('assets/Sounds/Slugs Winning/Speedster Win.mp3')
        self.trusty.win_sound = SoundLoader.load('assets/Sounds/Slugs Winning/Trusty Win.mp3')
        self.iffy.win_sound = SoundLoader.load('assets/Sounds/Slugs Winning/Iffy Win.mp3')
        self.slowpoke.win_sound = SoundLoader.load('assets/Sounds/Slugs Winning/Slowpoke Win.mp3') 
    ...
    def go(self, screen_manager): 
        ...
        # Accident   
        self.accident_expected = randint(0, 3) == 0 if self.race_number > 5 else False
    ...
    def finish_race(self, animation, slug):
        slug.finished = True    
        if self.speedster.finished and self.trusty.finished and self.iffy.finished and self.slowpoke.finished:
    ...
    def gameover(self, manual=False):
        ...
        # one player left in multi-player mode
            ...
            self.winner_text = f'The winner is {winner.name}, having started at ${winner.initial_money}, winning at ${winner.money}'
        
        # after a given number of races, after the game time has elapsed or if the game ended manually
        elif ((self.end_by_races and self.race_number == self.number_of_races)
              or (self.end_by_time and self.time_remaining_delta.seconds == 0)
              or manual):

            # The game over reason depends on why the game ends.
            if manual:
            ...
            if self.number_of_players == 1:
                you = self.players[0]
                self.winner_text = f"You were playing in 1-player mode.\nYou started at ${you.initial_money} and you're ending at ${you.money}."
               
            # multi-player mode
            else:
                max_money = (max(*self.players, key=lambda player: player.money)).money
                self.winners = [player for player in self.players if player.money == max_money]
                ...
                # joint winners
                    ...
                    for winner in self.winners:                        
                        self.winner_text += f'{winner.name}, having started at ${winner.initial_money}, winning at ${winner.money}\n'
    ...
    def reset_race(self):
        ...
        # Reset all slugs to their start positions.
        for slug in self.slugs:
            slug.reset()
