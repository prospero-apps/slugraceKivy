...
# We'll need the Translate class to move the eyes.
from kivy.graphics import Translate

class Accident(ABC):
    intro = 'BREAKING NEWS: '
    def __init__(self, name, headlines, sound, position = 0, slug = None, image = None, game=None):
        self.name = name
...
class BrokenLegAccident(Accident):
    ...
    def __init__(self, **kwargs):
        super().__init__(name=self.name, headlines=self.headlines, sound=self.sound, **kwargs)

    def happen(self):
        super().happen()
        self.slug.run_animation.stop(self.slug)
        self.slug.normal_image = self.slug.body.source 
        self.slug.body.source = 'atlas://assets/accidents/accidents/broken leg ' + self.slug.name.lower()
        self.slug.body.x += 10
...
class HeartAttackAccident(Accident):
    ...
    headlines = [
        ...
        "has a poor heart condition. If he hadn't stopped now, it could have killed him!",
        "beaten by cardiac infarction. He'd better go to hospital asap!",
        "almost killed by heart attack. Had a really narrow escape!",
        "beaten by his weak heart. He'd better have a rest!"]
    ...
    def happen(self):
        ...
        self.heart_animation = (Animation(size_hint = (.6, .6), duration = .2, t = 'out_back') 
                               + Animation(size_hint = (.8, .8), duration = .62, t = 'out_back'))
...
class AsleepAccident(Accident):
    ...
    headlines = [
        ...
        "always takes a short nap at this time of the day. Doesn't really matter what he's been doing!", 
        "knows how important it is to have a good day's sleep. Even if it's not the best time for that!"]
...
class BlindAccident(Accident):
    ...
    headlines = [
        ...
        "shouldn't have been reading in dark the night before the race. After going blind it's not easy to find the way!", 
...
class PuddleAccident(Accident):
    ...
    headlines = [
        ...
        "shouldn't have skipped his swimming lessons. Wouldn't be drowning in a puddle now!", 
        "has always neglected his swimming lessons. How fatally wrong he now turns out to have been!", 
...
class TurningBackAccident(Accident):
    ...
    headlines = [
        ...
        "seems to have more interesting stuff to do than racing. Let's hope he'll be back for the next race!", 
        "seems to have lost orientation. Well, how these little brains work!", 
        "has left his snack in the kitchen. He won't race when he's hungry, as he always puts it!"]
...
class RubberizedAccident(Accident):
    ...
    headlines = [
        ...
        "has never forgotten he was an eraser as a kid. Now his rubbery qualities come in handy!", 
